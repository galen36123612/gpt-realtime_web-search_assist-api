...
export async function POST(req: NextRequest) {
  try {
    ...
    while ((run.status === "queued" || run.status === "in_progress") && Date.now() < deadline) {
      ...
      if (!rr.ok) {
        return NextResponse.json(
          { error: "run_retrieve_failed", details: rr.json, thread_id, run_id },
          { status: 500 }
        );
      }
      run = rr.json;
    ...
    // 需要 action（例如 tool calls）— 先把狀態回去讓前端/RT 可以說明
    if (run.status === "requires_action") {
      return NextResponse.json(
        {
          error: "run_requires_action",
          status: run.status,
    ...
    if (!list.ok) {
      return NextResponse.json(
        { error: "messages_list_failed", details: list.json, thread_id, run_id },
        { status: 500 }
      );
    }
