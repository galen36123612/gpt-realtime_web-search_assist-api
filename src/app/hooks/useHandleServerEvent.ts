...
// 1230 realtime + web_search + assist run + useHandleServerEvent
...
export function useHandleServerEvent({
  ...
  const handleFunctionCall = async (functionCallParams: {
    ...
    try {
      ...
      // ✅ 2) web_search：打你後端 /api/web_search（保留原本）
      if (functionCallParams.name === "web_search") {
        const query = String(args?.query || "").trim();
        const recency_days =
          Number.isFinite(args?.recency_days) ? Number(args.recency_days) : 30;
      ...
      // ✅ 3) NEW: assistant_run：打你後端 /api/assistant_run（核心新增）
      if (functionCallParams.name === "assistant_run") {
        const input = String(
          args?.input ?? args?.query ?? args?.text ?? ""
        ).trim();

        // thread 延續：優先用 args.thread_id，否則用 localStorage
        let savedThread = "";
        try {
          savedThread =
            typeof window !== "undefined"
              ? localStorage.getItem("assistantThreadId") || ""
              : "";
        } catch {
          savedThread = "";
        }
        const thread_id = String(args?.thread_id ?? savedThread ?? "").trim();

        if (!input) {
          const errOut = { error: "Missing input for assistant_run" };
          addTranscriptBreadcrumb("function call result: assistant_run", errOut);
        ...
        // ✅ 保存 thread_id（延續同一個 Assistant thread）
        if (data?.thread_id && typeof data.thread_id === "string") {
          try {
            localStorage.setItem("assistantThreadId", data.thread_id);
          } catch {}
        }

        addTranscriptBreadcrumb("function call result: assistant_run", data);
        ...
        // ✅ 重要：再觸發一次 response.create，讓 Realtime 用結果回答（TTS + 文字）
        sendClientEvent({ type: "response.create" });
        return;
      }

      // ✅ 4) 原本 transferAgents 邏輯保留
      if (functionCallParams.name === "transferAgents") {
        const destinationAgent = args.destination_agent;
        const newAgentConfig =
          selectedAgentConfigSet?.find((a) => a.name === destinationAgent) ||
