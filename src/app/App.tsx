...
// 1230 final version backup rollback
...
// 1230 realtime + assistant run v2 
...
function AppContent() {
  ...
  // ✅ 重要：用 ref 解決「dc.message 裡閉包拿到舊 dataChannel」導致 tool output 送不出去
  ...
  // ✅ assistant thread（讓 assistant_run 可以延續同一個 thread）
  const [assistantThreadId, setAssistantThreadId] = useState<string>("");
  const assistantThreadIdRef = useRef<string>("");
  useEffect(() => {
    assistantThreadIdRef.current = assistantThreadId;
  }, [assistantThreadId]);
  ...
  // ✅（可選但推薦）用 user gesture 解鎖 autoplay，避免你覺得「TTS 沒聲音」
  ...
  async function connectToRealtime() {
    ...
    try {
      ...
      pc.ontrack = (e) => {
        if (audioElement.current) {
          audioElement.current.srcObject = e.streams[0];
          // 嘗試播放（可能被擋），真正解鎖靠 ensureAudioPlayback()
      ...
      dc.addEventListener("message", (e: MessageEvent) => {
        ...
        // 7️⃣ 助手回應完成 - ✅支援 function_call(web_search + assistant_run)
        const RESPONSE_DONE_EVENTS = ["response.done", "response.completed"];
        if (RESPONSE_DONE_EVENTS.includes(eventType)) {
          const outputItems = eventData?.response?.output || [];
          const functionCalls = Array.isArray(outputItems)
          ...
          // ✅ 7.0：如果這次 done 是工具呼叫（function_call），先做工具再回填結果
          if (functionCalls.length) {
            const callsToProcess = functionCalls.filter((c: any) => !processedToolCallIds.current.has(c.call_id));
            if (callsToProcess.length) {
              callsToProcess.forEach((c: any) => processedToolCallIds.current.add(c.call_id));
              ...
              void (async () => {
                try {
                  for (const call of callsToProcess) {
                    ...
                    // ✅ B) assistant_run（照 web_search 的流程：call → fetch → 回填 function_call_output）
                    if (call.name === "assistant_run") {
                      const input = String(args.input || args.query || args.text || "").trim();
                      const providedThread = String(args.thread_id || "").trim();
                      const threadToUse = providedThread || assistantThreadIdRef.current || undefined;
                      ...
                      if (!res.ok) {
                        ...
                        const answerPreview = String(data?.answer || "").slice(0, 200);
                        postLog({
                          role: "system",
                        ...
                        if (data?.thread_id && typeof data.thread_id === "string") {
                          // ✅ 更新 thread（state + ref）
                          assistantThreadIdRef.current = data.thread_id;
            ...
            // ⚠️ 這次 done 不當成最終回答（因為是 function_call）
            conversationState.current.currentAssistantResponse = {
              isActive: false,
              responseId: null,
              textBuffer: "",
...
- 當你需要使用我們已配置好的 OpenAI Assistant（長指令/固定風格/長期狀態）來回答時，先呼叫 assistant_run，把使用者問題原文帶入；工具回傳的 JSON 會有 answer 欄位，請以 answer 為主輸出。`;
    ...
    // ✅ assistant_run function tool（照 web_search 的方式加進 tools）
  ...
  const handleTalkButtonDown = () => {
    ...
    if (sessionStatus !== "CONNECTED" || dataChannelRef.current?.readyState !== "open") return;
    cancelAssistantSpeech();
    setIsPTTUserSpeaking(true);
    setIsListening(true);
    sendClientEvent({ type: "input_audio_buffer.clear" }, "clear PTT buffer");
  ...
  const handleTalkButtonUp = () => {
    ...
    if (sessionStatus !== "CONNECTED" || dataChannelRef.current?.readyState !== "open" || !isPTTUserSpeaking) return;
    setIsPTTUserSpeaking(false);
    setIsListening(false);
    sendClientEvent({ type: "input_audio_buffer.commit" }, "commit PTT");
    sendClientEvent({ type: "response.create" }, "trigger response PTT");
